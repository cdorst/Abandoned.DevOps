...
namespace DevOps.Abstractions.SourceCode.TypeDeclarations.EntityFramework.Services
{
        ...
        where TDbContext : SourceCodeTypeDeclarationsDbContext
    {
        ...
        {
            CacheKey = record => $"{nameof(SourceCode)}.{nameof(Field)}={record.AttributeListCollectionId}:{record.DocumentationCommentListId}:{record.IdentifierId}:{record.InitializerId}:{record.ModifierListId}:{record.TypeId}";
