...
namespace DevOps.Abstractions.SourceCode.TypeDeclarations.EntityFramework.Services
{
        ...
        where TDbContext : SourceCodeTypeDeclarationsDbContext
    {
        ...
        {
            CacheKey = record => $"{nameof(SourceCode)}.{nameof(Constructor)}={record.AttributeListCollectionId}:{record.BlockId}:{record.ConstructorBaseInitializerId}:{record.DocumentationCommentListId}:{record.IdentifierId}:{record.ModifierListId}:{record.ParameterListId}";
