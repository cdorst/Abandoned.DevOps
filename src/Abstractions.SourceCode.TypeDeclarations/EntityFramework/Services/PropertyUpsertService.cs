...
namespace DevOps.Abstractions.SourceCode.TypeDeclarations.EntityFramework.Services
{
        ...
        where TDbContext : SourceCodeTypeDeclarationsDbContext
    {
        ...
        {
            CacheKey = record => $"{nameof(SourceCode)}.{nameof(Property)}={record.AccessorListId}:{record.AttributeListCollectionId}:{record.DocumentationCommentListId}:{record.IdentifierId}:{record.ModifierListId}:{record.TypeId}";
